...
def post_content(TWITTER_POSTS, LINKEDIN_POSTS):
    ...
    while twitter_post_done < len(TWITTER_POSTS) or linkedin_post_done < len(LINKEDIN_POSTS):
        ...
        if current_time in TWITTER_POST_TIMES and current_time not in posted_today:
            if twitter_post_done < len(TWITTER_POSTS) and twitter_post_done < twitter_daily_limit:
                post_to_twitter(extract_post(TWITTER_POSTS[twitter_post_done])) #post with the current done index
                twitter_post_done += 1
                posted = True

        if current_time in LINKEDIN_POST_TIMES and current_time not in posted_today:
            if linkedin_post_done < len(LINKEDIN_POSTS) and linkedin_post_done < linkedin_daily_limit:
                ...
                if post:
                    ...
                    print(f"Error: Could not extract post at index {linkedin_index}")

        if posted:
            posted_today.add(current_time)
            print(f"✅ Posts so far → Twitter: {twitter_post_done}, LinkedIn: {linkedin_post_done}")

        # Check if daily limits are reached
        if twitter_post_done >= twitter_daily_limit and linkedin_post_done >= linkedin_daily_limit:
            print("🎯 Daily posting limits reached.")
        ...
        if twitter_post_done >= len(TWITTER_POSTS) and linkedin_post_done >= len(LINKEDIN_POSTS):
            print("🎯 All posts done for the current video.")
            break
        time.sleep(60)
...
def main():
    ...
    while True:
        ...
        print("🔄 Restarting next 2-day cycle...")
